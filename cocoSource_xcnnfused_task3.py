...
class ImageCaptionModel(nn.Module):
    ...
    def forward(self, cnn_features, x_tokens, is_train: bool, current_hidden_state=None) -> tuple:
        ...
        if current_hidden_state is None:
            ...
            print("Shape before cat LSTM: ", initial_hidden_state.shape)
            if self.cell_type == "LSTM":
                initial_hidden_state = torch.cat((initial_hidden_state, initial_hidden_state), dim=2)
            print("Shape initial hidden state: ", initial_hidden_state.shape)
            #torch.zeros((self.num_rnn_layers, x_tokens.shape[0], self.hidden_state_sizes))
        else:
...
class RNN(nn.Module):
    def __init__(self, input_size, hidden_state_size, num_rnn_layers, cell_type='GRU'):
        ...
        self.cells = nn.ModuleList([{self.cell_type == "RNN" : RNNsimpleCell(self.hidden_state_size, self.input_size),
                                     self.cell_type == "GRU" : GRUCell(self.hidden_state_size, self.input_size),
                                     self.cell_type == "LSTM": LSTMCell(self.hidden_state_size, self.input_size)}.get(True)\
                                    if i == 0 else
                                    {self.cell_type == "RNN" : RNNsimpleCell(self.hidden_state_size, self.hidden_state_size),
                                     self.cell_type == "GRU" : GRUCell(self.hidden_state_size, self.hidden_state_size),
                                     self.cell_type == "LSTM": LSTMCell(self.hidden_state_size, self.input_size)}.get(True)\
    ...
    def forward(self, tokens, processed_cnn_features, initial_hidden_state, output_layer: nn.Linear,
        ...
        if is_train:
            for i in range(sequence_length):
                ...
                for layer in range(self.num_rnn_layers):
                    output = self.cells[layer](input, hidden_states[layer])
                    hidden_states[layer] = output
                    input = output
                logits_sequence.append(output_layer(input))
        ...
        # if not training
            ...
            for i in range(sequence_length):
                ...
                logits_sequence.append(logits_i)
                predictions = torch.argmax(logits_i, dim=1)
                input = embedding_layer(input=predictions)


        current_hidden_state = torch.stack(hidden_states)
        logits = torch.stack(logits_sequence, dim=1)  # Convert the sequence of logits to a tensor
...
class LSTMCell(nn.Module):
    def __init__(self, hidden_state_size: int, input_size: int):
        ...
        self.weight_f = nn.Parameter(
            torch.randn(n, 2*hidden_state_size)/(torch.sqrt(n)))
